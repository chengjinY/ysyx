...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();
	...
	INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw , I, R(dest) = (uint32_t)(src1 + src2));
	INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc	, U, R(dest) = src1 + s->pc);
	INSTPAT("??????? ????? ????? 000 ????? 11000 11", beqz  , B, if (src1 == src2) s->dnpc = s->pc + dest);
	INSTPAT("??????? ????? ????? ??? ????? 00101 11", bne 	, B, if (src1 != src2) s->dnpc = s->pc + dest);
	INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal   , J, R(dest) = s->pc + 4; s->dnpc = s->pc + src1);
	INSTPAT("??????? ????? ????? 010 ????? 11001 11", jalr  , I, R(dest) = s->pc + 4; s->dnpc = ((src1 + src2) & ~1));
