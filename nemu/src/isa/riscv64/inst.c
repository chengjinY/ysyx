...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw 	, R, R(dest) = SEXT(S64(BITS(src1, 31, 0)) / S64(BITS(src2, 31, 0)), 32));
	INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal   , J, R(dest == 32 ? 1 : dest) = s->pc + 4; s->dnpc = s->pc + src1);
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr  , I, R(dest == 32 ? 1 : dest) = s->pc + 4; s->dnpc = ((src1 + src2) & (~U64(1))));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu  	, I, R(dest) = U64(Mr(src1 + src2, 1)));
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld   	, I, R(dest) = Mr(src1 + src2, 8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh   	, I, R(dest) = SEXT(Mr(src1 + src2, 2), 32));
