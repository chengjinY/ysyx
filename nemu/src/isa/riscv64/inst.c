...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw 	, R, R(dest) = SEXT(S64(BITS(src1, 31, 0)) / S64(BITS(src2, 31, 0)), 32));
	INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal   , J, R(dest == 0 ? 1 : dest) = s->pc + 4; s->pc = s->pc + src1);
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr  , I, R(dest == 0 ? 1 : dest) = s->pc + 4; s->pc = ((src1 + src2) & (~U64(1))); Log("%lx %lx %lx", src1, src2, R(dest == 0 ? 1 : dest)));
