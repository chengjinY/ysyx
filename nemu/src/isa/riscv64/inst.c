...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();
	...
	INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw , I, R(dest) = SEXT(src1 + src2, 32); Log("addiw: %ld %ld %ld", src1, src2, R(dest)));
	INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw  , R, R(dest) = (uint64_t)SEXT(src1 + src2, 32); Log("addw: %ld %ld %ld", src1, src2, R(dest)));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw   	, I, R(dest) = Mr(src1 + src2, 4); Log("lw: %ld %ld %ld", src1, src2, R(dest)));
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd   	, S, Mw(src1 + dest, 8, src2)); //
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu	, I, R(dest) = (src1 < src2); Log("sltiu: %ld %ld %ld", src1, src2, R(dest)));
