...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();
	...
	INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw , I, R(dest) = SEXT(BITS(src1 + src2, 31, 0), 32); Log("%ld %lld", R(dest), BITS(src1 + src2, 31, 0)));
	INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw  , R, R(dest) = SEXT(BITS(src1 + src2, 31, 0), 32); Log("%ld %lld", R(dest), BITS(src1 + src2, 31, 0)));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw 	, R, R(dest) = SEXT(S64(BITS(src1, 31, 0)) / S64(BITS(src2, 31, 0)), 32));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu  	, I, R(dest) = U64(Mr(src1 + src2, 1)));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu	, I, R(dest) = U64(src1) < U64(src2); Log("%lu %lu", U64(src1), U64(src2)));
