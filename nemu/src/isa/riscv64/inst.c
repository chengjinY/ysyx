...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();
	...
	INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw , I, R(dest) = SEXT(src1 + src2, 32); Log("addiw: %lu %lu %lu", src1, src2, R(dest)));
	INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw  , R, R(dest) = src1 + src2; Log("addw: %lu %lu %lu", src1, src2, R(dest)));
	INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc	, U, R(dest) = src1 + s->pc); //
	INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq   , B, if (src1 == src2) s->dnpc = s->dnpc + dest);
	INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne 	, B, if (src1 != src2) s->dnpc = s->dnpc + dest);
