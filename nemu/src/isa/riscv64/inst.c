...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();
	...
	INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw , I, R(dest) = (src1 + src2) & BITMASK(32); Log("addiw: %ld %ld %ld", src1, src2, R(dest)));
	INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw  , R, R(dest) = (src1 + src2) & BITMASK(32); Log("addw: %ld %ld %ld", src1, src2, R(dest)));
